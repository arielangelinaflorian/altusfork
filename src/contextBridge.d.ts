...
declare global {
  interface Window {
    ...
    sessionTools: {
      getDiagnostics: (partition: string) => Promise<{
        partition: string;
        normalizedPartition: string;
        cacheSizeBytes: number;
        partitionDirSizeBytes: number;
        partitionFileCount: number;
        cookiesCount: number;
        partitionPath: string;
        partitionPathExists: boolean;
      }>;
    ...
    perfDebug: {
      enabled: boolean;
      mark: (event: string, detail?: Record<string, unknown>) => void;
    };
  }
}
